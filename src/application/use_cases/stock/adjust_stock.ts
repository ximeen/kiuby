import { MovementReason, MovementType, StockMovement } from "@domain/entities/stock/stock_movement.entity";
import { IStockMovementRepository, IStockRepository } from "@domain/entities/stock/stock_repository";
...
export class AdjustStockUseCase {
    constructor(private stockRepo: IStockRepository, private movementRepo: IStockMovementRepository){}

    async execute(input: AdjustStockInput):Promise<void> {
        const newQuantity = Quantity.create(input.newQuantity);

        const stock = await this.stockRepo.findByProductAndWarehouse(input.productId, input.warehouseId);
        if(!stock){
